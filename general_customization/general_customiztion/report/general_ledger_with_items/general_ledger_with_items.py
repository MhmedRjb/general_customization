...
def execute(filters=None):
    ...
    if filters.get("show_item_details"):
        message = _(
            "Notice: This report is in its beta phase and is subject to ongoing improvements. Users are advised to review the data carefully and report any discrepancies."
        )
    ...
    if filters.get("account") or (filters.get("party") and len(filters.party) == 1):
        ...
        if filters.get("account"):
            ...
            gle_currency = frappe.db.get_value(
                "GL Entry",
                {
                    "party_type": filters.party_type[0],
            ...
            if gle_currency:
                ...
                account_currency = (
                    ...
                    if filters.party_type[0] in ["Employee", "Shareholder", "Member"]
                        ...
                        filters.party_type[0], filters.party[0], "default_currency"
                    )
                )
...
def get_result(filters, account_details):
    ...
    data = get_data_with_opening_closing(
        filters, account_details, accounting_dimensions, gl_entries
    )
...
def get_conditions(filters):
    ...
    if filters.get("include_default_book_entries"):
        if filters.get("finance_book"):
            ...
            conditions.append(
                "(finance_book in (%(company_fb)s, '') OR finance_book IS NULL)"
            )
    else:
        if filters.get("finance_book"):
            conditions.append(
    ...
    for d in accounts:
        ...
        if account:
            children = frappe.get_all(
                "Account",
                filters={"lft": [">=", account.lft], "rgt": ["<=", account.rgt]},
            )
...
def get_data_with_opening_closing(
    ...
    totals, entries = get_accountwise_gle(
        filters, accounting_dimensions, gl_entries, gle_map
    )
...
def get_accountwise_gle(filters, accounting_dimensions, gl_entries, gle_map):
    ...
    def update_value_in_dict(data, key, gle):
        ...
        if filters.get("add_values_in_transaction_currency") and key not in [
            "opening",
            "closing",
            "total",
        ]:
            ...
            data[
                key
            ].credit_in_transaction_currency += gle.credit_in_transaction_currency
